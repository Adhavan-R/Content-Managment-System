...
<body>
            ...
            <?php
                ...
                $check_query = mysqli_query($connect, "SHOW TABLES LIKE 'mou';");
                $check = mysqli_fetch_array($check_query);
                if ($check == null) {
                  $create_query = mysqli_query($connect, " CREATE TABLE mou( 
                ...
                $rows = mysqli_query($connect, "SELECT * FROM mou ;");
                ...
                if ($flag == 1) {
                    ...
                    for($i = 0; $i < $num_rows-1; $i++){
                        $insert_query = mysqli_query(
                          $connect,
                          "INSERT INTO mou( title,company,amount,date,year,file) VALUES ('".$myarray[$i]['title']."' , '".$myarray[$i]['company']."' , '".$myarray[$i]['amount']."','".$myarray[$i]['date']."','".$myarray[$i]['year']."','".$myarray[$i]['file']."');"
                        );
                    }
                    ...
                    if ($truncate_query == true) {
                        ...
                        echo '<a href="/Project/10.MOU/MOU.php"> Ok </a>';
                    } else {
