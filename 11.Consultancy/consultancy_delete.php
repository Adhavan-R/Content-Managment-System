...
<body>
            ...
            <?php
                ...
                $check_query = mysqli_query($connect, "SHOW TABLES LIKE 'consultancy';");
                $check = mysqli_fetch_array($check_query);
                if ($check == null) {
                  $create_query = mysqli_query($connect, " CREATE TABLE consultancy( 
                ...
                $rows = mysqli_query($connect, "SELECT * FROM consultancy ;");
                ...
                if ($flag == 1) {
                    ...
                    $truncate_query = mysqli_query(
                      $connect,
                      "TRUNCATE TABLE consultancy;"
                    );

                    for($i = 0; $i < $num_rows-1; $i++){
                        $insert_query = mysqli_query(
                          $connect,
                          "INSERT INTO consultancy( title,company,amount,date,year,file) VALUES ('".$myarray[$i]['title']."' , '".$myarray[$i]['company']."' , '".$myarray[$i]['amount']."','".$myarray[$i]['date']."','".$myarray[$i]['year']."','".$myarray[$i]['file']."');"
                        );
                    }
                    ...
                    if ($truncate_query == true) {
                        ...
                        echo '<a href="/Project/11.Consultancy/Consultancy.php"> Ok </a>';
                    } else {
