    ...
    if ($_FILES['file']['name'] != "") {
        ...
        if (!move_uploaded_file($_FILES['file']['tmp_name'], $file_name)) {
            ...
            $insert_query = mysqli_query(
                $connect,
                "UPDATE publication SET title = '$title',company = '$company',amount = '$amount',date = '$date',year = '$year', file = '$file' WHERE s_no = $s_no;"
            );
        }
    } else {
        $insert_query = mysqli_query(
            $connect,
            "UPDATE publication SET title = '$title',company = '$company',amount = '$amount',date = '$date',year = '$year' WHERE s_no = $s_no;"
        );
    }
